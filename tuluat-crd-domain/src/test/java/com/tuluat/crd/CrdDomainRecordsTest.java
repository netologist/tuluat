...
class CrdDomainRecordsTest {
    ...
    void testAiAgentCrAndStatus() {
        ...
        var spec = new AiAgentSpec(
            new ProviderRef("openai-provider", "tuluat-system"),
            "gpt-4o",
            "System prompt",
            "User prompt",
            List.of(new SkillDefinition("calculator", "Math tool", true, Map.of())),
            ...
            new GuardrailsConfig(
                ...
                new OutputValidationConfig(true, 0.8)
            ),
            new A2aConfig(true, "http://a2a-gateway:8080"),
            new IngressSpec(true, "agent.example.com", "/", "Prefix", "nginx", Map.of(), new TlsSpec("tls-secret", List.of("agent.example.com"))),
            2
        );
        agent.setSpec(spec);
        ...
        // Status factories
        AiAgentStatus readyStatus = AiAgentStatus.ready("http://agent.example.com", List.of("calculator"), "gpt-4o", "Agent active", 1L);
    ...
    void testLlmProviderCr() {
        ...
        var spec = new LlmProviderSpec(
            "OPENAI",
            "https://api.openai.com/v1",
            new SecretKeyRef("openai-secret", "api-key"),
            "gpt-4o",
            0.7,
            4096,
            0.0025,
