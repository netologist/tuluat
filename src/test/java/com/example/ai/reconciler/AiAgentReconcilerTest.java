...
import com.example.ai.crd.agent.AiAgentSpec;
import com.example.ai.crd.agent.IngressSpec;
import com.example.ai.crd.agent.ProviderRef;
import com.example.ai.crd.agent.SkillDefinition;
import com.example.ai.crd.provider.LlmProvider;
import com.example.ai.crd.provider.LlmProviderSpec;
...
class AiAgentReconcilerTest {
    ...
    void testReconcileAiAgentFull() {
        ...
        agent.setMetadata(new ObjectMetaBuilder().withName("support-agent").withNamespace("default").withUid("uid-123").withGeneration(1L).build());
        agent.setSpec(new AiAgentSpec(
            ...
            new IngressSpec(true, "agent.example.com", "/", "Prefix", "nginx", Map.of(), null),
            1
        ));
        ...
        assertEquals("http://agent.example.com", agent.getStatus().ingressUrl());
