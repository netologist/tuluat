package com.example.ai.reconciler;

import com.example.ai.crd.agent.AiAgent;
import com.example.ai.crd.agent.AiAgentStatus;
...
/**
 * JOSDK Reconciler for managing AiAgent Custom Resources and creating Deployment, Service, & Ingress.
 */
@Component
...
public class AiAgentReconciler implements Reconciler<AiAgent> {
    ...
    public UpdateControl<AiAgent> reconcile(AiAgent agent, Context<AiAgent> context) {
        ...
        try {
            ...
            if (spec == null) {
                agent.setStatus(AiAgentStatus.failed("AiAgent spec cannot be null", agent.getMetadata().getGeneration()));
            ...
            if (spec.providerRef() != null && spec.providerRef().name() != null) {
                ...
                if (provider == null) {
                    ...
                    agent.setStatus(AiAgentStatus.reconciling(
                        "Waiting for LlmProvider '" + pName + "' to be ready",
            ...
            List<String> activeSkills = spec.skills().stream()
            ...
            String readyMessage = String.format("AiAgent '%s' successfully reconciled and listening at %s", name, ingressUrl);
            agent.setStatus(AiAgentStatus.ready(
                ingressUrl,
                activeSkills,
                effectiveModel,
                readyMessage,
            ...
            agent.setStatus(AiAgentStatus.failed("Reconciliation failure: " + e.getMessage(), agent.getMetadata().getGeneration()));
            return UpdateControl.patchStatus(agent);
        }
    }
    ...
    private void reconcileDeployment(AiAgent agent, OwnerReference ownerRef, String ns) {
        ...
        Deployment deployment = new DeploymentBuilder()
            ...
            .endMetadata()
            .withNewSpec()
                .withReplicas(agent.getSpec().replicas())
                .withNewSelector()
                ...
                .withNewTemplate()
                    ...
                    .withNewSpec()
                        .addNewContainer()
                            .withName("ai-agent-app")
                            .withImage("k8s-crd-ai-operator:latest") // Uses operator runtime image
                            .addNewPort()
                            ...
                            .addNewEnv()
                                .withName("AGENT_NAME")
                                .withValue(agent.getMetadata().getName())
                            .endEnv()
                        .endContainer()
                    .endSpec()
                .endTemplate()
    ...
    private String reconcileIngress(AiAgent agent, OwnerReference ownerRef, String ns) {
        ...
        String host = (ingressSpec.host() != null && !ingressSpec.host().isBlank()) ? ingressSpec.host() : "ai-agent.local";
        ...
        var pathRule = new HTTPIngressPathBuilder()
            .withPath(path)
            .withPathType(ingressSpec.pathType())
            .withNewBackend()
                .withNewService()
                    .withName(svcName)
                    .withNewPort()
        ...
        var rule = new IngressRuleBuilder()
            .withHost(host)
            .withHttp(new HTTPIngressRuleValueBuilder().withPaths(pathRule).build())
            .build();
        ...
        if (ingressSpec.tls() != null) {
            var tls = new IngressTLSBuilder()
                .withSecretName(ingressSpec.tls().secretName())
                .withHosts(ingressSpec.tls().hosts())
                .build();
        ...
        return String.format("%s://%s%s", protocol, host, path.endsWith("/") ? path.substring(0, path.length() - 1) : path);
    }
}
