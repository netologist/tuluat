...
public class ModelGateway {
     ...
     *
     * @param answer       model output text
     * @param modelUsed    effective model (after fallback resolution)
     * @param inputTokens  prompt tokens
     * @param outputTokens completion tokens
     * @param costUsd      estimated cost in USD
     * @param usedFallback true when the primary route failed and a fallback succeeded
     */
     ...
     *
     * @param prompt             assembled Spring AI prompt
     * @param provider           primary LlmProvider (may be null)
     * @param agentModel         agent-level model override (may be null)
     * @param providerResolver   resolves fallback providers by name
     * @param budgetLimitUsd     max spend for this execution; null disables budget
     * @param agentName          agent id for budget tracking
     * @return call result
     */
    public GatewayCallResult invoke(
        Prompt prompt,
        ...
        if (budgetLimitUsd != null && agentName != null) {
            ...
            if (budget.spentUsd() >= budgetLimitUsd) {
                log.warn("ModelGateway budget exhausted for agent [{}]: spent=${}, limit=${}",
                    agentName, budget.spentUsd(), budgetLimitUsd);
        ...
        if (provider != null && spec != null && spec.fallbacks() != null) {
            for (ModelFallback fallback : spec.fallbacks()) {
                ...
                if (fbResult != null) {
                    log.info("ModelGateway fell back to provider [{}] model [{}]",
                        fallback.providerName(), fbModelName);
                    recordSpend(agentName, fbResult.costUsd());
                    return new GatewayCallResult(
                        fbResult.answer(), fbModelName,
                        fbResult.inputTokens(), fbResult.outputTokens(),
                        fbResult.costUsd(), true
                    );
                }
            }
        }

        throw new ModelGatewayException("All model routes failed for agent: " + (agentName != null ? agentName : "unknown"));
    }
    ...
    public static class BudgetExceededException extends RuntimeException {
        public BudgetExceededException(String agentName, double spentUsd, double limitUsd) {
            super(String.format("Budget exceeded for agent [%s]: spent=$%.4f limit=$%.4f", agentName, spentUsd, limitUsd));
        }
    }
