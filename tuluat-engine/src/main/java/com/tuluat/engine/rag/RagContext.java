 ...
 *            the original query
 * @param retrieved
 *            top-K chunks ordered by descending similarity
 */
public record RagContext(String query, List<RetrievedChunk> retrieved) {
	...
	public String toPromptBlock() {
		...
		for (RetrievedChunk chunk : retrieved) {
			sb.append(String.format("- [%s #%d (sim %.2f)]: %s%n", chunk.sourceRef(), chunk.chunkIndex(),
