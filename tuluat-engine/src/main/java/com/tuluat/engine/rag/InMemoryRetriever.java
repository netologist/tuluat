...
/**
 * In-memory retriever: brute-force cosine similarity over embedded chunks
 * (ADR 008). Used in unit tests and as the default when pgvector is not
 * configured. Active when {@code tuluat.rag.retriever=memory} (default).
 * Chunks are stored with their pre-computed embeddings.
 */
@Component
