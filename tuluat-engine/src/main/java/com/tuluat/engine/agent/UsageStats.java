...
public record UsageStats(@JsonProperty("inputTokens") int inputTokens, @JsonProperty("outputTokens") int outputTokens,
		@JsonProperty("totalTokens") int totalTokens, @JsonProperty("estimatedCostUsd") double estimatedCostUsd,
		@JsonProperty("latencyMs") long latencyMs) {
	/**
	 * Calculates UsageStats and estimates USD cost based on model token rates per
	 * 1,000,000 tokens.
	 */
	...
	/**
	 * Returns a copy of these stats with the gateway-computed cost (USD) instead of
	 * the built-in model-rate estimate.
	 */
