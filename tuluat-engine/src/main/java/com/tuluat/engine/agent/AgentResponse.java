...
public record AgentResponse(
    ...
    @JsonProperty("guardrailStatus") String guardrailStatus
) {
    public static AgentResponse create(String agentName, String model, String systemPrompt, String answer, List<SkillResult> skills, UsageStats usage) {
        return new AgentResponse(agentName, model, systemPrompt, answer, skills, usage, java.time.Instant.now().toString(), null);
    }

    public static AgentResponse blocked(String agentName, String filterName, String reason) {
        return new AgentResponse(
            agentName, "blocked", "N/A",
            "Request blocked by guardrail [" + filterName + "]: " + reason,
            List.of(), UsageStats.calculate(0, 0, "blocked", 0),
