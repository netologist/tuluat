...
class EmbabelAgentRunnerTest {
	...
	void testExecuteGoal() {
		AgentResponse mockResponse = AgentResponse.create("web-researcher-agent", "deepseek-chat", "DEEPSEEK",
				"Goal achieved: Research completed", List.of(), UsageStats.calculate(10, 10, "deepseek-chat", 100L));

		when(agentExecutionService.executeAgent(eq("web-researcher-agent"), anyString(), any()))
				.thenReturn(mockResponse);

		AgentResponse response = runner.executeGoal("web-researcher-agent", "Research CRD Operators",
				Map.of("depth", "high"));
