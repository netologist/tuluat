...
/**
 * Orchestrates the guardrails pipeline for one agent execution:
 * pre-execution filters in order (PII masking, prompt injection defense),
 * then post-execution output validation.
 */
@Service
public class GuardrailPipeline {
    ...
    public GuardrailPipeline(
        List<PreExecutionFilter> preExecutionFilters,
     ...
     *
     * @throws GuardrailBlockedException when a filter blocks the prompt
     */
    ...
    public List<String> getActiveFilterNames(GuardrailsConfig config) {
        return preExecutionFilters.stream()
            .filter(f -> f.isEnabled(config))
            .map(GuardrailFilter::getFilterName)
            .toList();
    }
}
