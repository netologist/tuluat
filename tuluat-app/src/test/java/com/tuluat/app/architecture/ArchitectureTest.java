...
public class ArchitectureTest {
	...
	private static final ArchCondition<JavaClass> HAVE_CREATION_TIMESTAMP = new ArchCondition<>(
		...
		public void check(JavaClass javaClass, ConditionEvents events) {
			boolean found = javaClass.getAllFields().stream().anyMatch(f -> f.isAnnotatedWith(CreationTimestamp.class));
			if (!found) {
	...
	private static final ArchCondition<JavaClass> NO_AUTOWIRED_PARAMS = new ArchCondition<>(
			"have no @Autowired on constructor parameters") {
		@Override
		public void check(JavaClass javaClass, ConditionEvents events) {
			boolean hasAutowired = javaClass.getConstructors().stream()
					.anyMatch(c -> c.getParameters().stream().anyMatch(p -> p.isAnnotatedWith(Autowired.class)));
			if (hasAutowired) {
				events.add(SimpleConditionEvent.violated(javaClass,
						javaClass.getName() + " has @Autowired on constructor params"));
			}
		}
	};
	public static final ArchRule reconcilers_in_operator_package = classes().that().implement(Reconciler.class).should()
			.resideInAPackage("..operator.reconciler..").allowEmptyShould(true);

	@ArchTest
	public static final ArchRule reconcilers_suffixed_and_annotated = classes().that().implement(Reconciler.class)
			.should().haveSimpleNameEndingWith("Reconciler").andShould().beAnnotatedWith(Component.class).orShould()
			.beAnnotatedWith(Service.class).allowEmptyShould(true);
	...
	@ArchTest
	public static final ArchRule embabel_ai_in_engine_package = classes().that().haveSimpleNameContaining("Embabel")
			.or().haveSimpleNameEndingWith("Engine").should().resideInAPackage("..engine..").allowEmptyShould(true);
	...
	@ArchTest
	public static final ArchRule crd_has_kind_plural_shortnames = classes().that().areAssignableTo(CustomResource.class)
			.should().beAnnotatedWith(Kind.class).andShould().beAnnotatedWith(Plural.class).andShould()
			.beAnnotatedWith(ShortNames.class).allowEmptyShould(true);

	@ArchTest
	public static final ArchRule no_legacy_date_calendar = noFields().should().haveRawType(java.util.Date.class)
			.orShould().haveRawType(java.util.Calendar.class).allowEmptyShould(true);

	@ArchTest
	public static final ArchRule no_explicit_logger_field = noFields().should().haveRawType(org.slf4j.Logger.class)
			.allowEmptyShould(true);
