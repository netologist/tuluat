...
public class WorkflowController {
    ...
    public ResponseEntity<List<Map<String, Object>>> listWorkflows(@RequestParam(required = false) String namespace) {
        ...
        List<AiWorkflow> items = kubernetesClient.resources(AiWorkflow.class)
                .inNamespace(ns)
                .list()
                .getItems();

        if (items.isEmpty()) {
            items = kubernetesClient.resources(AiWorkflow.class)
                    .inNamespace("default")
                    .list()
                    .getItems();
        }
        ...
        List<Map<String, Object>> response = items.stream().map(wf -> {
            ...
            map.put("nodeCount", (wf.getSpec() != null && wf.getSpec().getNodes() != null) ? wf.getSpec().getNodes().size() : 0);
    ...
    public ResponseEntity<Map<String, Object>> getWorkflow(@PathVariable String name,
        ...
        String ns = (namespace != null && !namespace.isBlank()) ? namespace : "tuluat-system";
        AiWorkflow wf = kubernetesClient.resources(AiWorkflow.class)
                .inNamespace(ns)
                .withName(name)
                .get();

        if (wf == null) {
            wf = kubernetesClient.resources(AiWorkflow.class)
                    .inNamespace("default")
                    .withName(name)
                    .get();
        }
