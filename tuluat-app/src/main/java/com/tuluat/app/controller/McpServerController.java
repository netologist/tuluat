...
public class McpServerController {
    ...
    public ResponseEntity<List<Map<String, Object>>> listMcpServers(@RequestParam(required = false) String namespace) {
        ...
        List<McpServer> items = kubernetesClient != null ?
                kubernetesClient.resources(McpServer.class).inNamespace(ns).list().getItems() : List.of();
        ...
        List<Map<String, Object>> response = items.stream().map(this::mapMcpServerToSecureView).collect(Collectors.toList());
    ...
    public ResponseEntity<Map<String, Object>> getMcpServer(@PathVariable String name,
        ...
        McpServer server = kubernetesClient != null ?
                kubernetesClient.resources(McpServer.class).inNamespace(ns).withName(name).get() : null;
        ...
        if (server == null) {
            return getSampleMcpServers().stream()
    ...
    public ResponseEntity<Map<String, Object>> createOrOverrideMcpServer(@RequestBody Map<String, Object> request) {
        ...
        SecretKeyRef secretRef = (newApiKey != null && !newApiKey.isBlank()) ?
                new SecretKeyRef(name + "-secret", "api-key") : null;
        ...
        if (kubernetesClient != null) {
            try {
                kubernetesClient.resources(McpServer.class).inNamespace("tuluat-system").resource(server).createOrReplace();
    ...
    private Map<String, Object> mapMcpServerToSecureView(McpServer s) {
        ...
        if (spec != null) {
            ...
            map.put("authStatus", "NONE".equalsIgnoreCase(spec.authType()) ? "Public (No Auth)" : "Configured (Secret)");
    ...
    private List<String> getExportedToolsForServer(String name) {
        if (name == null) return List.of("tool_execution");
    ...
    private List<Map<String, Object>> getSampleMcpServers() {
        return List.of(
            Map.of(
                "name", "postgres-pgvector-mcp",
                "namespace", "tuluat-system",
                "endpoint", "http://postgres-pgvector:5432/sse",
                "transport", "SSE",
                "authType", "NONE",
                "timeoutSeconds", 30,
                "description", "PostgreSQL Vector Database Memory MCP Server",
                "authStatus", "Public (No Auth)",
                "apiKeyMasked", "N/A",
                "exportedTools", List.of("pgvector_query_order_history", "semantic_vector_search", "similarity_knn_match")
            ),
            Map.of(
                ...
                "authType", "API_KEY",
                "timeoutSeconds", 30,
                "description", "Stripe & Ledger Payment Settlement MCP Server",
                "authStatus", "Configured (Secret)",
                "apiKeyMasked", "••••••••••••••••",
                "exportedTools", List.of("stripe_charge_customer_balance", "ledger_verify_account", "refund_transaction")
            ),
            Map.of(
                "name", "warehouse-mcp",
                "namespace", "tuluat-system",
                ...
                "authType", "API_KEY",
                "timeoutSeconds", 30,
                "description", "Warehouse Logistics & Shipping Dispatch MCP Server",
                "authStatus", "Configured (Secret)",
                "apiKeyMasked", "••••••••••••••••",
                "exportedTools", List.of("inventory_reserve_and_dispatch", "shipping_label_generator", "stock_check")
            ),
            Map.of(
                "name", "github-mcp",
                "namespace", "tuluat-system",
                "endpoint", "http://github-mcp:8080/sse",
                "transport", "SSE",
                "authType", "API_KEY",
                "timeoutSeconds", 30,
                "description", "GitHub Repositories & Issue Tracker MCP Server",
                "authStatus", "Configured (Secret)",
                "apiKeyMasked", "••••••••••••••••",
                "exportedTools", List.of("create_issue", "pull_request_review", "get_file_contents")
            )
        );
    }
}
