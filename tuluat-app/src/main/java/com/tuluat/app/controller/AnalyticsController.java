...
public class AnalyticsController {
    ...
    @Autowired
    public AnalyticsController(KubernetesClient kubernetesClient,
    ...
    public ResponseEntity<List<Map<String, Object>>> listProviders(@RequestParam(required = false) String namespace) {
        ...
        List<LlmProvider> items = kubernetesClient.resources(LlmProvider.class)
                .inNamespace(ns)
                .list()
                .getItems();

        if (items.isEmpty()) {
            items = kubernetesClient.resources(LlmProvider.class)
                    .inNamespace("default")
                    .list()
                    .getItems();
        }
        ...
        List<Map<String, Object>> response = items.stream().map(provider -> {
            ...
            map.put("costPer1kInputTokens", provider.getSpec() != null ? provider.getSpec().costPer1kInputTokens() : 0.0015);
            map.put("costPer1kOutputTokens", provider.getSpec() != null ? provider.getSpec().costPer1kOutputTokens() : 0.0030);
    ...
    public ResponseEntity<Map<String, Object>> getAnalyticsOverview() {
        ...
        long waitingApprovals = allSessions.stream().filter(s -> "WAITING_APPROVAL".equalsIgnoreCase(s.getStatus())).count();
        ...
        List<Map<String, Object>> modelBreakdown = List.of(
                Map.of("model", "gpt-4o", "provider", "OPENAI", "sessions", Math.max(1, totalSessions / 2), "costUsd", Math.round(totalCostUsd * 0.6 * 10000.0) / 10000.0),
                Map.of("model", "deepseek-chat", "provider", "DEEPSEEK", "sessions", Math.max(0, totalSessions / 4), "costUsd", Math.round(totalCostUsd * 0.25 * 10000.0) / 10000.0),
                Map.of("model", "llama3.2", "provider", "OLLAMA", "sessions", Math.max(0, totalSessions / 4), "costUsd", 0.0)
        );
