...
public class AiAgentController {
    ...
    public ResponseEntity<List<Map<String, Object>>> listAgents(@RequestParam(required = false) String namespace) {
        ...
        List<AiAgent> items = kubernetesClient.resources(AiAgent.class)
                .inNamespace(ns)
                .list()
                .getItems();

        if (items.isEmpty()) {
            items = kubernetesClient.resources(AiAgent.class)
                    .inNamespace("default")
                    .list()
                    .getItems();
        }
    ...
    public ResponseEntity<Map<String, Object>> getAgent(@PathVariable String name,
        ...
        if (agent == null) {
            agent = kubernetesClient.resources(AiAgent.class)
                    .inNamespace("default")
                    .withName(name)
                    .get();
        }
    ...
    public ResponseEntity<List<Map<String, Object>>> getAgentLogs(@PathVariable String name) {
        ...
        List<Map<String, Object>> agentLogs = allLogs.stream()
                .filter(l -> l.getMessage() != null && l.getMessage().contains(name))
                .map(l -> {
