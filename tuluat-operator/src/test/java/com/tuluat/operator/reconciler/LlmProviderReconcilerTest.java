...
class LlmProviderReconcilerTest {
	...
	void testReconcileSuccess() {
		Secret secret = new SecretBuilder().withNewMetadata().withName("openai-secret").withNamespace("default")
				.endMetadata().withStringData(Map.of("api-key", "secret-val")).build();
		doReturn(secret).when(secretResourceMock).get();

		var provider = new LlmProvider();
		provider.setMetadata(new ObjectMetaBuilder().withName("openai-provider").withNamespace("default")
				.withGeneration(1L).build());
		provider.setSpec(new LlmProviderSpec("OPENAI", "https://api.openai.com/v1",
				new SecretKeyRef("openai-secret", "api-key"), "gpt-4o", 0.7, 2048, 0.0, 0.0, List.of()));
	...
	void testReconcileMissingSecret() {
		...
		var provider = new LlmProvider();
		provider.setMetadata(new ObjectMetaBuilder().withName("missing-sec-provider").withNamespace("default")
				.withGeneration(1L).build());
		provider.setSpec(new LlmProviderSpec("OPENAI", "https://api.openai.com/v1",
				new SecretKeyRef("non-existent-secret", "api-key"), "gpt-4o", 0.7, 2048, 0.0, 0.0, List.of()));
