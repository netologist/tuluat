...
import com.tuluat.crd.provider.LlmProviderStatus;
...
public class LlmProviderReconciler implements Reconciler<LlmProvider> {
	...
	public UpdateControl<LlmProvider> reconcile(LlmProvider resource, Context<LlmProvider> context) {
		...
		try {
			...
			if (spec == null) {
				resource.setStatus(
						LlmProviderStatus.error("Spec cannot be null", resource.getMetadata().getGeneration()));
			...
			if (spec.apiKeySecretRef() != null) {
				...
				if (secret == null) {
					...
					resource.setStatus(
							LlmProviderStatus.pending("Waiting for Secret '" + secretName + "' to be created",
									resource.getMetadata().getGeneration()));
					return UpdateControl.patchStatus(resource);
				}
			}

			String msg = String.format("LLM Provider [%s] is Ready (type: %s, defaultModel: %s)", name,
					spec.providerType(), spec.defaultModel());
			...
			resource.setStatus(LlmProviderStatus.error("Reconciliation error: " + e.getMessage(),
					resource.getMetadata().getGeneration()));
			return UpdateControl.patchStatus(resource);
		}
	}
