 ...
 * On reconcile the client connection is (re)registered; on delete it is
 * removed.
 */
@Component
	...
	public UpdateControl<McpServer> reconcile(McpServer resource, Context<McpServer> context) {
		...
		try {
			...
			if (spec == null || spec.endpoint() == null || spec.endpoint().isBlank()) {
				resource.setStatus(
						McpServerStatus.error("Spec.endpoint cannot be null", resource.getMetadata().getGeneration()));
				return UpdateControl.patchStatus(resource);
			}

			clientRegistry.registerFromCr(resource);

			String msg = String.format("MCP Server [%s] registered (endpoint: %s, transport: %s)", name,
					spec.endpoint(), spec.transport());
			...
			resource.setStatus(McpServerStatus.error("Reconciliation error: " + e.getMessage(),
